                ...
                if (type === 'TCPWRAP') {
                    process.nextTick(() => {
                        ...
                        resource.owner.once('lookup', (err, address, family, host) => {
                            this.log.debug('Circuit breaker lookup', host, asyncId);

                            if (this.registry.has(host)) {
                                breaker = this.registry.get(host);
                                const foo = breaker.check();
                                this.log.debug('Circuit breaker lookup check', host, asyncId, foo, breaker.failures);
